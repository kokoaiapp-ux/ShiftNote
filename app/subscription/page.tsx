...
import { beginFirstTimeFlow, completeFirstTimeFlow, hasCompletedFirstTimeFlow, hasCompletedOnboarding, isFirstTimeFlowPending, trackPaywallEvent } from "@/lib/first-time-flow";
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    ...
    if (requestedFirstTime && requestedStage === "entry") {
      if (hasCompletedFirstTimeFlow() || hasCompletedOnboarding()) { router.replace("/dashboard"); return; }
      beginFirstTimeFlow();
  ...
  async function buy(planId: string) {
    ...
    if (!auth.configured) { if (flow?.active) completeFirstTimeFlow(); trackPaywallEvent("purchase_completed", { plan: planId, mode: "preview" }); router.push("/dashboard"); return; }
    ...
    try {
      ...
      if (flow?.active) completeFirstTimeFlow();
