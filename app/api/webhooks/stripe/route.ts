...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (["customer.subscription.created", "customer.subscription.updated", "customer.subscription.deleted"].includes(event.type)) { const subscription = event.data.object as Stripe.Subscription; const row = await syncStripeSubscription(subscription); if (row && revenueCatConfigured()) await submitStripeSubscription(row.user_id, subscription.id).catch(() => console.error("RevenueCat receipt sync failed", { subscriptionId: subscription.id })); if (event.type === "customer.subscription.deleted" || subscription.cancel_at_period_end) await sendTikTokServerEvent({ event: "SubscriptionCancellation", eventId: `stripe-cancel-${subscription.id}-${subscription.items.data[0]?.current_period_end || 0}`, properties: { content_id: row?.plan || "shiftnote_pro", content_type: "product" }, path: "/billing" }).catch(() => console.error("TikTok cancellation event delivery failed", { subscriptionId: subscription.id })); }
    if (["invoice.paid", "invoice.payment_succeeded"].includes(event.type)) { const invoice = event.data.object as Stripe.Invoice; const customerId = typeof invoice.customer === "string" ? invoice.customer : invoice.customer?.id; if (customerId && invoice.amount_paid > 0) await markSubscriptionPaidByCustomer(admin, customerId, new Date((invoice.status_transitions.paid_at || invoice.created) * 1000)); if (event.type === "invoice.paid" && invoice.amount_paid > 0) { const renewal = invoice.billing_reason === "subscription_cycle"; await sendTikTokServerEvent({ event: renewal ? "SubscriptionRenewal" : "CompletePayment", eventId: `stripe-invoice-${invoice.id}`, properties: { currency: invoice.currency.toUpperCase(), value: invoice.amount_paid / 100, content_id: "shiftnote_pro", content_type: "product" }, path: "/billing" }).catch(() => console.error("TikTok payment event delivery failed", { invoiceId: invoice.id })); } }
