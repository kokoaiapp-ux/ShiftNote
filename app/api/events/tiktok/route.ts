...
const allowed = new Set(["PageView", "CompleteRegistration", "Login", "CompleteOnboarding", "ViewContent", "InitiateCheckout", "SubscriptionCancellation"]);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await sendTikTokServerEvent({ event: body.event, eventId: body.eventId, properties, path: body.path });
    return NextResponse.json({ accepted: true, configured: result.configured });
