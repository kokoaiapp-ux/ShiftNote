...
export function AuthCard({ mode }: { mode: "login" | "signup" }) {
  ...
  async function submit(event:FormEvent){event.preventDefault();if(auth.configured&&mode==="signup"&&password.length<8){setMessage({type:"error",text:"Use a password with at least 8 characters."});return;}setBusy(true);setMessage(null);try{if(!auth.configured){router.push(target);return;}if(mode==="signup"){const result=await auth.signUpEmail(name.trim(),email.trim(),password);if(result.emailConfirmationRequired){setMessage({type:"success",text:"Account created. Check your email and click the verification link. You will then continue to onboarding."});}else router.push(target);}else{await auth.signInEmail(email.trim(),password);router.push(target);}}catch(error){setMessage({type:"error",text:friendlyAuthError(error)});}finally{setBusy(false)}}
  async function oauth(provider:"google"|"apple"){setBusy(true);setMessage(null);try{await(provider==="google"?auth.signInGoogle(target):auth.signInApple(target));}catch(error){setMessage({type:"error",text:friendlyAuthError(error)});setBusy(false)}}
