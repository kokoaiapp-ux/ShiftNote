...
export function friendlyAuthError(error: unknown) { const message = error instanceof Error ? error.message : "Authentication could not be completed."; if (/invalid login credentials/i.test(message)) return "The email or password is incorrect."; if (/email not confirmed/i.test(message)) return "Verify your email address before signing in."; if (/already registered|already exists/i.test(message)) return "An account already uses this email."; if (/password/i.test(message) && /weak|characters/i.test(message)) return "Use a password with at least 8 characters."; return message; }
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function oauth(provider: "google" | "apple", next = "/dashboard") { const callback = new URL("/auth/callback", location.origin); callback.searchParams.set("next", safeNext(next)); const { error } = await requireSupabase().auth.signInWithOAuth({ provider, options: { redirectTo: callback.toString() } }); if (error) throw error; }
  const value: AuthContextValue = {
    ...
    async signUpEmail(name, email, password) { const { data, error } = await requireSupabase().auth.signUp({ email, password, options: { data: { display_name: name }, emailRedirectTo: `${location.origin}/auth/callback?next=${encodeURIComponent("/onboarding")}` } }); if (error) throw error; return { emailConfirmationRequired: !data.session }; },
    signInGoogle(next) { return oauth("google", next); },
    signInApple(next) { return oauth("apple", next); },
    async resetPassword(email) { const { error } = await requireSupabase().auth.resetPasswordForEmail(email, { redirectTo: `${location.origin}/auth/callback?next=${encodeURIComponent("/update-password")}` }); if (error) throw error; },
