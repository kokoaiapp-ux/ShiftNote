...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data } = supabaseClient.auth.onAuthStateChange((event, session) => { if (event === "SIGNED_IN") { consumePendingAuthEvent(); consumePendingTikTokAuthEvent(); consumePendingMetaAuthEvent(); } void load(session?.user ?? null); });
    return () => { active = false; data.subscription.unsubscribe(); };
  }, []);

  async function oauth(provider: "google" | "apple", next = "/dashboard") { const registration = safeNext(next).startsWith("/onboarding"); setPendingTikTokAuthEvent(registration ? "CompleteRegistration" : "Login"); setPendingMetaAuthEvent(registration ? "CompleteRegistration" : "Login"); const callback = new URL("/auth/callback", browserAppUrl()); callback.searchParams.set("next", safeNext(next)); const { error } = await requireSupabase().auth.signInWithOAuth({ provider, options: { redirectTo: callback.toString() } }); if (error) throw error; }
