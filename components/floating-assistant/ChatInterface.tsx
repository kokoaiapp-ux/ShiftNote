...
export function ChatInterface({ messages, status, error, onSend, onClose, onNewConversation, floating, workspace = false }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className={cn("shrink-0 border-t border-[var(--border)] bg-[var(--card)] p-4", workspace && "p-3 lg:pb-12")}>
        ...
        {speech.isSupported && speech.inputDevices.length > 0 && recordingPhase === "idle" && (
          <label className="mb-2 hidden items-center gap-2 text-[10px] text-[var(--muted-foreground)] sm:flex">
            ...
            <select aria-label="Recording microphone" className={cn("min-w-0 flex-1 rounded-lg border border-[var(--border)] bg-[var(--background)] px-2.5 py-2 text-xs text-[var(--foreground)] outline-none focus:border-[var(--primary)]", workspace && "py-1.5")} onChange={(event) => speech.setSelectedDeviceId(event.target.value)} value={speech.selectedDeviceId}>
        ...
        <form className={cn("flex flex-wrap items-end gap-1 rounded-2xl border border-[var(--border)] bg-[var(--background)] p-2 focus-within:border-[var(--primary)]", workspace && "rounded-xl p-1.5")} onSubmit={submit}>
          ...
          <textarea aria-label="Message ShiftNote" autoComplete="off" className={cn("order-first max-h-32 min-h-10 w-full min-w-0 flex-none basis-full resize-none overflow-y-auto bg-transparent px-2 py-2.5 text-sm leading-5 outline-none placeholder:text-[var(--muted-foreground)]", workspace && "max-h-24 min-h-8 py-1.5")} onChange={(event) => { setInput(event.target.value); event.currentTarget.style.height = "0px"; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, workspace ? 96 : 128)}px`; }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={recordingPhase === "recording" ? "Recording…" : recordingPhase === "transcribing" ? "Transcribing…" : "Describe the clinical facts…"} rows={1} value={input} />
          ...
          <button aria-label="Send message" className="ml-auto grid size-10 shrink-0 place-items-center rounded-xl bg-[var(--primary)] text-white disabled:opacity-[0.65]" disabled={!canSend} type="submit"><Send className="size-4" /></button>
        </form>
        <p className={cn("mt-2 text-center text-[10px] text-[var(--muted-foreground)]", workspace && "mt-1.5")}>Review and attest all AI-generated documentation before clinical use.</p>
      </div>
      {templateDialogOpen && (
