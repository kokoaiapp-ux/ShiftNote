  ...
  return (
    ...
    >
      ...
      <div className={cn("shrink-0 border-t border-[var(--border)] bg-[var(--card)] p-4", workspace && "lg:pr-32")}>
        ...
        <form className="flex flex-wrap items-end gap-1 rounded-2xl border border-[var(--border)] bg-[var(--background)] p-2 focus-within:border-[var(--primary)]" onSubmit={submit}>
          ...
          <textarea aria-label="Message ShiftNote" autoComplete="off" className="order-first max-h-32 min-h-10 w-full min-w-0 flex-none basis-full resize-none overflow-y-auto bg-transparent px-2 py-2.5 text-sm leading-5 outline-none placeholder:text-[var(--muted-foreground)]" onChange={(event) => { setInput(event.target.value); event.currentTarget.style.height = "0px"; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 128)}px`; }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={recordingPhase === "recording" ? "Recording…" : recordingPhase === "transcribing" ? "Transcribing…" : "Describe the clinical facts…"} rows={1} value={input} />
