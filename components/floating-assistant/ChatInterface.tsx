  ...
  return (
    ...
    >
      ...
      <div className="border-t border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <form className="flex flex-wrap items-end gap-1 rounded-2xl border border-[var(--border)] bg-[var(--background)] p-2 focus-within:border-[var(--primary)] sm:flex-nowrap" onSubmit={submit}>
          ...
          <textarea aria-label="Message ShiftNote" autoComplete="off" className="order-first max-h-32 min-h-10 w-full min-w-0 flex-none basis-full resize-none overflow-y-auto bg-transparent px-2 py-2.5 text-sm leading-5 outline-none placeholder:text-[var(--muted-foreground)] sm:order-none sm:w-auto sm:flex-1 sm:basis-auto" onChange={(event) => { setInput(event.target.value); event.currentTarget.style.height = "0px"; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 128)}px`; }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={recordingPhase === "recording" ? "Recording…" : recordingPhase === "transcribing" ? "Transcribing…" : "Describe the clinical facts…"} rows={1} value={input} />
          ...
          <button aria-label="Send message" className="ml-auto grid size-10 shrink-0 place-items-center rounded-xl bg-[var(--primary)] text-white disabled:opacity-[0.65] sm:ml-0" disabled={!canSend} type="submit"><Send className="size-4" /></button>
        </form>
