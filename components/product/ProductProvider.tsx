...
export function ProductProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateWithAI = useCallback(async (note: SavedNote, newInformation: string, operation: "update" | "summarize" = "update") => { const accessToken = await auth.accessToken(); const response = await fetch("/api/chat/update", { method: "POST", headers: { "Content-Type": "application/json", ...(accessToken ? { Authorization: `Bearer ${accessToken}` } : {}) }, body: JSON.stringify({ existingNote: note.preview, newInformation, modeId: note.modeId, templateId: note.templateId, operation }) }); if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; throw new Error(payload?.error || "Unable to update the documentation."); } return response.text(); }, [auth]);
  ...
  const summarizeHistory = useCallback(async (id: string, existingNote?: string) => { if (historyReadOnly) throw new Error("History is read-only because ShiftNote Pro access has expired."); const note = history.find((item) => item.id === id); if (!note) throw new Error("Saved documentation was not found."); const updated = await updateWithAI({ ...note, preview: existingNote || note.preview }, "", "summarize"); updateHistoryNote(id, { preview: updated }, "ai"); return updated; }, [history, historyReadOnly, updateHistoryNote, updateWithAI]);
