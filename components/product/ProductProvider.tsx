...
export function ProductProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveToHistory = useCallback((note: SavedNote) => { if (historyReadOnly) return; setHistory((items) => [note, ...items.filter((item) => item.id !== note.id)]); if (auth.user) void persistNote(requireSupabase(), auth.user.id, note).then((ids) => setHistory((items) => items.map((item) => item.id === note.id ? { ...item, ...ids } : item))).catch(reportPersistenceError); }, [auth.user, historyReadOnly]);
  const addFavorite = useCallback((note: SavedNote) => { if (favorites.some((item) => item.preview === note.preview && item.modeId === note.modeId && item.templateId === note.templateId)) return; setFavorites((items) => [note, ...items]); if (auth.user) void persistNote(requireSupabase(), auth.user.id, note).then(async (ids) => { const favoriteId = await addFavoriteRecord(requireSupabase(), auth.user!.id, ids.messageId, note.id); setFavorites((items) => items.map((item) => item.id === note.id ? { ...item, id: favoriteId, ...ids } : item)); }).catch(reportPersistenceError); }, [auth.user, favorites]);
  const deleteFavorite = useCallback((id: string) => { setFavorites((items) => items.filter((item) => item.id !== id)); if (auth.user) void deleteFavoriteRecord(requireSupabase(), auth.user.id, id).catch(reportPersistenceError); }, [auth.user]);
