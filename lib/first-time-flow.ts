export const PRIMARY_PAYWALL_PURCHASE_FLAG = "hasPurchasedPrimaryPaywall";
...
export function hasPurchasedPrimaryPaywall() {
  return typeof window !== "undefined" && localStorage.getItem(PRIMARY_PAYWALL_PURCHASE_FLAG) === "true";
}
export function beginFirstTimeFlow() { localStorage.setItem(FIRST_TIME_FLOW_KEY, "active"); }
export function isFirstTimeFlowPending() { return typeof window !== "undefined" && localStorage.getItem(FIRST_TIME_FLOW_KEY) === "active" && !hasPurchasedPrimaryPaywall(); }
...
export function markPrimaryPaywallPurchased() { localStorage.setItem(PRIMARY_PAYWALL_PURCHASE_FLAG, "true"); localStorage.removeItem(FIRST_TIME_FLOW_KEY); }
